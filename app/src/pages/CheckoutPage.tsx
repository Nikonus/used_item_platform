...
export const CheckoutPage = () => {
  ...
  const [buyerAddressResult, setBuyerAddressResult] =
  ...
  const [sellerAddressResult] =
    useState<AddressResult | null>(
      item?.sellerAddress &&
        item?.sellerAddressLat &&
        item?.sellerAddressLng
        ? {
  ...
  useEffect(() => {
    ...
    if (buyerInputRef.current) {
      cleanupBuyer = createAutocomplete(
  ...
  useEffect(() => {
    const computeDistance = async () => {
      ...
      const dist = await calculateDistance(
        {
          lat: buyerAddressResult.lat,
          lng: buyerAddressResult.lng,
        },
        {
          lat: sellerAddressResult.lat,
          lng: sellerAddressResult.lng,
        },
        GOOGLE_MAPS_API_KEY,
      )
      ...
      const { data: order, error: orderError } =
        await supabase
          ...
          .insert({
            ...
            buyer_address:
              buyerAddressResult.formatted,

            buyer_address_lat:
              buyerAddressResult.lat,

            buyer_address_lng:
              buyerAddressResult.lng,

            seller_address:
              sellerAddressResult.formatted,

            seller_address_lat:
              sellerAddressResult.lat,

            seller_address_lng:
              sellerAddressResult.lng,
